...
### CodeMirror gotchas
- **The bundled `codemirror.bundle.js` is curated.** Built one-shot via `esbuild entry.js --bundle --format=iife --global-name=CM --minify --target=es2020` against the public `@codemirror/*` packages — no in-repo build step. Exports the full common set: core (`EditorState`, `EditorSelection`, `EditorView`, `keymap`, `lineNumbers`, `Decoration`, `drawSelection`, `highlightActiveLine`), commands (`defaultKeymap`, `history`, `historyKeymap`, `indentWithTab`), language (`syntaxHighlighting`, `defaultHighlightStyle`, `bracketMatching`, `foldGutter`, `foldKeymap`, `codeFolding`), markdown lang + theme (`markdown`, `oneDark`), search (`search`, `searchKeymap`, `openSearchPanel`, `closeSearchPanel`, `findNext`, `findPrevious`, `replaceNext`, `replaceAll`, `setSearchQuery`, `SearchQuery`, `getSearchQuery`, `highlightSelectionMatches`), autocomplete (`autocompletion`, `completionKeymap`, `closeBrackets`, `closeBracketsKeymap`), plus a preloaded `codeLanguages` array for fenced-block highlighting in: javascript/ts, python, yaml, json, html, css, shell, go, rust, sql, dockerfile, toml.
- **Find/replace** is the standard CM6 panel — `search({ top: true })` extension + `searchKeymap` register Ctrl/Cmd+F, Ctrl/Cmd+H, F3, etc. natively. Match highlighting uses the built-in `.cm-searchMatch` / `.cm-searchMatch-selected` classes.
- **Bracket auto-pairing** via `closeBrackets()` covers `[]`, `()`, `{}`, `""`, `''`, `` `` ``. Quote auto-pairing is suppressed when the cursor is after a word char (avoids `you're` → `you'`re` surprises).
- **Heading fold** via `foldGutter()` — markdown's language support emits fold ranges per heading section automatically.
- **To rebuild the bundle**: create a temp dir with `entry.js` (re-exports the imports listed above) and `package.json` (pinned `@codemirror/*` versions), `npm install`, then `npx esbuild entry.js --bundle --format=iife --global-name=CM --minify --target=es2020 > codemirror.bundle.js` and copy it back to `static/`. Don't redirect stderr — `npm` writes warnings there and they end up prepended to the output.
- **Selections**: `view.dispatch({selection: {anchor, head}})` is the simplest form; `EditorSelection.create([…])` is available for multi-cursor cases.
...
This project is indexed by GitNexus as **vaultreader** (1266 symbols, 1892 relationships, 84 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
...
| Resource | Use for |
|----------|---------|
| `gitnexus://repo/vaultreader/context` | Codebase overview, check index freshness |
| `gitnexus://repo/vaultreader/clusters` | All functional areas |
| `gitnexus://repo/vaultreader/processes` | All execution flows |
| `gitnexus://repo/vaultreader/process/{name}` | Step-by-step execution trace |
